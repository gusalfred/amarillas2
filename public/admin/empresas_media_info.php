...
//
// Table class for empresas_media
//
class cempresas_media extends cTable {
	var $id_empresa_media;
	var $id_empresa;
	var $archivo;
	var $nombre;
	var $orden;
	...
	function __construct() {
		...
		$this->TableVar = 'empresas_media';
		$this->TableName = 'empresas_media';
		...
		// id_empresa_media
		$this->id_empresa_media = new cField('empresas_media', 'empresas_media', 'x_id_empresa_media', 'id_empresa_media', '`id_empresa_media`', '`id_empresa_media`', 19, -1, FALSE, '`id_empresa_media`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_empresa_media->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_empresa_media'] = &$this->id_empresa_media;

		// id_empresa
		$this->id_empresa = new cField('empresas_media', 'empresas_media', 'x_id_empresa', 'id_empresa', '`id_empresa`', '`id_empresa`', 19, -1, FALSE, '`id_empresa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_empresa->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_empresa'] = &$this->id_empresa;

		// archivo
		$this->archivo = new cField('empresas_media', 'empresas_media', 'x_archivo', 'archivo', '`archivo`', '`archivo`', 200, -1, TRUE, '`archivo`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['archivo'] = &$this->archivo;

		// nombre
		$this->nombre = new cField('empresas_media', 'empresas_media', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nombre'] = &$this->nombre;

		// orden
		$this->orden = new cField('empresas_media', 'empresas_media', 'x_orden', 'orden', '`orden`', '`orden`', 19, -1, FALSE, '`orden`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`empresas_media`";
	}
	...
	// Update Table
	var $UpdateTable = "`empresas_media`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('id_empresa_media', $rs))
				ew_AddFilter($where, ew_QuotedName('id_empresa_media') . '=' . ew_QuotedValue($rs['id_empresa_media'], $this->id_empresa_media->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`id_empresa_media` = @id_empresa_media@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_empresa_media->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_empresa_media@", ew_AdjustSql($this->id_empresa_media->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "empresas_media_list.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("empresas_media_view.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("empresas_media_view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "empresas_media_add.php?" . $this->UrlParm($parm);
		else
			return "empresas_media_add.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("empresas_media_edit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("empresas_media_add.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("empresas_media_delete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_empresa_media->CurrentValue)) {
			$sUrl .= "id_empresa_media=" . urlencode($this->id_empresa_media->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_empresa_media"]; // id_empresa_media
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_empresa_media->CurrentValue = $key;
   ...
   // Common render codes
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->id_empresa_media->Exportable) $Doc->ExportCaption($this->id_empresa_media);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->id_empresa_media->Exportable) $Doc->ExportField($this->id_empresa_media);
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
		header('Location: empresas_view.php?tab=4&id_empresa='.$rsnew['id_empresa']);
		exit;
	}
	...
	function Row_Updated($rsold, &$rsnew) {
		...
		//echo "Row Updated";
		header("Location: empresas_view.php?tab=4&id_empresa=".$rsold['id_empresa'] );
		die();
	}
	...
	function Row_Deleted(&$rs) {
		...
		//echo "Row Deleted";
		header("Location: empresas_view.php?tab=4&id_empresa=".$rs['id_empresa'] );
		die();
	}
