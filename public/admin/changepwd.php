...
class cchangepwd extends cempleados {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["empleados"])) $GLOBALS["empleados"] = &$this;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'empleados');
		}
	...
	function Page_Main() {
		...
		if ($bPostBack && $bValidate) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$rsold = $rs->fields;
					if (ew_ComparePassword($rsold['clave'], $this->OldPassword)) {
						$bValidPwd = TRUE;
						$bValidPwd = $this->User_ChangePassword($rsold, $sUsername, $this->OldPassword, $this->NewPassword);
						if ($bValidPwd) {
							$rsnew = array('clave' => $this->NewPassword); // Change Password
							$rs->Close();
...
<?php } ?>
	<div class="form-group">
		...
		<div class="col-sm-10"><input type="password" name="opwd" id="opwd" class="form-control ewControl"></div>
	</div>
	<div class="form-group">
		<label class="col-sm-2 control-label ewLabel" for="npwd"><?php echo $Language->Phrase("NewPassword") ?></label>
		<div class="col-sm-10"><input type="password" name="npwd" id="npwd" class="form-control ewControl"></div>
	</div>
	<div class="form-group">
		<label class="col-sm-2 control-label ewLabel" for="cpwd"><?php echo $Language->Phrase("ConfirmPassword") ?></label>
		<div class="col-sm-10"><input type="password" name="cpwd" id="cpwd" class="form-control ewControl"></div>
	</div>
	<div class="form-group">
