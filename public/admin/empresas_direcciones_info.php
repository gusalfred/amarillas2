...
$empresas_direcciones = NULL;

//
// Table class for empresas_direcciones
//
class cempresas_direcciones extends cTable {
	var $id_empresa_direccion;
	var $id_empresa;
	var $direccion;
	...
	function __construct() {
		...
		$this->TableVar = 'empresas_direcciones';
		$this->TableName = 'empresas_direcciones';
		...
		$this->id_empresa_direccion = new cField('empresas_direcciones', 'empresas_direcciones', 'x_id_empresa_direccion', 'id_empresa_direccion', '`id_empresa_direccion`', '`id_empresa_direccion`', 19, -1, FALSE, '`id_empresa_direccion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_empresa_direccion->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_empresa_direccion'] = &$this->id_empresa_direccion;

		// id_empresa
		$this->id_empresa = new cField('empresas_direcciones', 'empresas_direcciones', 'x_id_empresa', 'id_empresa', '`id_empresa`', '`id_empresa`', 19, -1, FALSE, '`id_empresa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_empresa->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_empresa'] = &$this->id_empresa;

		// direccion
		$this->direccion = new cField('empresas_direcciones', 'empresas_direcciones', 'x_direccion', 'direccion', '`direccion`', '`direccion`', 200, -1, FALSE, '`direccion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['direccion'] = &$this->direccion;

		// codigo_departamento
		$this->codigo_departamento = new cField('empresas_direcciones', 'empresas_direcciones', 'x_codigo_departamento', 'codigo_departamento', '`codigo_departamento`', '`codigo_departamento`', 200, -1, FALSE, '`codigo_departamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['codigo_departamento'] = &$this->codigo_departamento;

		// codigo_provincia
		$this->codigo_provincia = new cField('empresas_direcciones', 'empresas_direcciones', 'x_codigo_provincia', 'codigo_provincia', '`codigo_provincia`', '`codigo_provincia`', 200, -1, FALSE, '`codigo_provincia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['codigo_provincia'] = &$this->codigo_provincia;

		// codigo_distrito
		$this->codigo_distrito = new cField('empresas_direcciones', 'empresas_direcciones', 'x_codigo_distrito', 'codigo_distrito', '`codigo_distrito`', '`codigo_distrito`', 200, -1, FALSE, '`codigo_distrito`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['codigo_distrito'] = &$this->codigo_distrito;

		// telefonos
		$this->telefonos = new cField('empresas_direcciones', 'empresas_direcciones', 'x_telefonos', 'telefonos', '`telefonos`', '`telefonos`', 200, -1, FALSE, '`telefonos`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['telefonos'] = &$this->telefonos;

		// latitud
		$this->latitud = new cField('empresas_direcciones', 'empresas_direcciones', 'x_latitud', 'latitud', '`latitud`', '`latitud`', 200, -1, FALSE, '`latitud`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['latitud'] = &$this->latitud;

		// longitud
		$this->longitud = new cField('empresas_direcciones', 'empresas_direcciones', 'x_longitud', 'longitud', '`longitud`', '`longitud`', 200, -1, FALSE, '`longitud`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`empresas_direcciones`";
	}
	...
	// Update Table
	var $UpdateTable = "`empresas_direcciones`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('id_empresa_direccion', $rs))
				ew_AddFilter($where, ew_QuotedName('id_empresa_direccion') . '=' . ew_QuotedValue($rs['id_empresa_direccion'], $this->id_empresa_direccion->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`id_empresa_direccion` = @id_empresa_direccion@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_empresa_direccion->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_empresa_direccion@", ew_AdjustSql($this->id_empresa_direccion->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "empresas_direcciones_list.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("empresas_direcciones_view.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("empresas_direcciones_view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "empresas_direcciones_add.php?" . $this->UrlParm($parm);
		else
			return "empresas_direcciones_add.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("empresas_direcciones_edit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("empresas_direcciones_add.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("empresas_direcciones_delete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_empresa_direccion->CurrentValue)) {
			$sUrl .= "id_empresa_direccion=" . urlencode($this->id_empresa_direccion->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_empresa_direccion"]; // id_empresa_direccion
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_empresa_direccion->CurrentValue = $key;
   ...
   // Common render codes
		...
		// id_empresa_direccion
		$this->id_empresa_direccion->LinkCustomAttributes = "";
		$this->id_empresa_direccion->HrefValue = "";
		$this->id_empresa_direccion->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// id_empresa_direccion
		$this->id_empresa_direccion->EditAttrs["class"] = "form-control";
		$this->id_empresa_direccion->EditCustomAttributes = "";
		$this->id_empresa_direccion->EditValue = $this->id_empresa_direccion->CurrentValue;
		$this->id_empresa_direccion->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->codigo_departamento->Exportable) $Doc->ExportCaption($this->codigo_departamento);
					if ($this->codigo_provincia->Exportable) $Doc->ExportCaption($this->codigo_provincia);
					if ($this->codigo_distrito->Exportable) $Doc->ExportCaption($this->codigo_distrito);
					...
					if ($this->id_empresa_direccion->Exportable) $Doc->ExportCaption($this->id_empresa_direccion);
					if ($this->id_empresa->Exportable) $Doc->ExportCaption($this->id_empresa);
					if ($this->direccion->Exportable) $Doc->ExportCaption($this->direccion);
					if ($this->codigo_departamento->Exportable) $Doc->ExportCaption($this->codigo_departamento);
					if ($this->codigo_provincia->Exportable) $Doc->ExportCaption($this->codigo_provincia);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->codigo_departamento->Exportable) $Doc->ExportField($this->codigo_departamento);
						if ($this->codigo_provincia->Exportable) $Doc->ExportField($this->codigo_provincia);
						if ($this->codigo_distrito->Exportable) $Doc->ExportField($this->codigo_distrito);
						...
						if ($this->id_empresa_direccion->Exportable) $Doc->ExportField($this->id_empresa_direccion);
						if ($this->id_empresa->Exportable) $Doc->ExportField($this->id_empresa);
						if ($this->direccion->Exportable) $Doc->ExportField($this->direccion);
						if ($this->codigo_departamento->Exportable) $Doc->ExportField($this->codigo_departamento);
						if ($this->codigo_provincia->Exportable) $Doc->ExportField($this->codigo_provincia);
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
		header("Location: empresas_view.php?tab=2&id_empresa=".$rsnew['id_empresa'] );
		exit;
	}
	...
	function Row_Updated($rsold, &$rsnew) {
		...
		//echo "Row Updated";
		header("Location: empresas_view.php?tab=2&id_empresa=".$rsold['id_empresa'] );
		exit;
	}
	...
	function Row_Deleted(&$rs) {
		...
		//echo "Row Deleted";
		header("Location: empresas_view.php?tab=2&id_empresa=".$rs['id_empresa'] );
		exit;
	}
