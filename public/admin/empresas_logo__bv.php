...
$empresas_logo_blobview = NULL; // Initialize page object first

class cempresas_logo_blobview extends cempresas {
	...
	// Page object name
	var $PageObjName = 'empresas_logo_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'empresas');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('empresas', 'x_logo');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('logo');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('logo')));
				if (trim(strval($this->Recordset->fields('logo'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('logo') . "\"");
				}
...
// Create page object
if (!isset($empresas_logo_blobview)) $empresas_logo_blobview = new cempresas_logo_blobview();

// Page init
$empresas_logo_blobview->Page_Init();

// Page main
$empresas_logo_blobview->Page_Main();
?>
<?php
$empresas_logo_blobview->Page_Terminate();
?>
