...
class cuserpriv extends cempleados_niveles {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["empleados_niveles"])) $GLOBALS["empleados_niveles"] = &$this;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'empleados_niveles');
	...
	function Page_Main() {
		...
		$Breadcrumb->Add("list", "empleados_niveles", "empleados_niveles_list.php", "", "empleados_niveles");
		...
		if (@$_POST["a_edit"] == "") {
			...
			if (@$_GET["id_empleado_nivel"] <> "") {
				...
				$this->Page_Terminate("empleados_niveles_list.php"); // Return to list
			}
			if ($this->id_empleado_nivel->QueryStringValue == "-1") {
			...
			$this->id_empleado_nivel->setFormValue($_POST["x_id_empleado_nivel"]);
	...
	function EditRow() {
		...
		for ($i = 0; $i < $this->TableNameCount; $i++) {
			$Sql = "SELECT * FROM " . EW_USER_LEVEL_PRIV_TABLE . " WHERE " . 
				...
				EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . " = " . $this->id_empleado_nivel->CurrentValue;
			$rs = $conn->Execute($Sql);
			if ($rs && !$rs->EOF) {
				...
				$Sql = "INSERT INTO " . EW_USER_LEVEL_PRIV_TABLE . " (" . EW_USER_LEVEL_PRIV_TABLE_NAME_FIELD . ", " . EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . ", " . EW_USER_LEVEL_PRIV_PRIV_FIELD . ") VALUES ('" . ew_AdjustSql($this->TableList[$i][4] . $this->TableList[$i][0]) . "', " . $this->id_empleado_nivel->CurrentValue . ", " . $this->Privileges[$i] . ")";
				$conn->Execute($Sql);
			}
			if ($rs)
				$rs->Close();
...
<p><?php echo $Language->Phrase("UserLevel") ?><?php echo $Security->GetUserLevelName($empleados_niveles->id_empleado_nivel->CurrentValue) ?>(<?php echo $empleados_niveles->id_empleado_nivel->CurrentValue ?>)</p>
<?php
...
<input type="hidden" name="x_id_empleado_nivel" id="x_id_empleado_nivel" value="<?php echo $empleados_niveles->id_empleado_nivel->CurrentValue ?>">
...
for ($i = 0; $i < $userpriv->TableNameCount; $i++) {
	$userpriv->TempPriv = $Security->GetUserLevelPrivEx($userpriv->TableList[$i][4] . $userpriv->TableList[$i][0], $empleados_niveles->id_empleado_nivel->CurrentValue);
