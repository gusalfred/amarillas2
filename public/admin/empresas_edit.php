...
class cempresas_edit extends cempresas {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->id_empresa->CurrentValue == "")
			$this->Page_Terminate("empresas_list.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// documento_tipo
			$this->documento_tipo->SetDbValueDef($rsnew, $this->documento_tipo->CurrentValue, NULL, $this->documento_tipo->ReadOnly);

			// documento_numero
			$this->documento_numero->SetDbValueDef($rsnew, $this->documento_numero->CurrentValue, NULL, $this->documento_numero->ReadOnly);
			...
			// logo
			if (!($this->logo->ReadOnly) && !$this->logo->Upload->KeepFile) {
			...
			// meta_descripcion
			$this->meta_descripcion->SetDbValueDef($rsnew, $this->meta_descripcion->CurrentValue, NULL, $this->meta_descripcion->ReadOnly);

			// meta_palabras_clave
			$this->meta_palabras_clave->SetDbValueDef($rsnew, $this->meta_palabras_clave->CurrentValue, NULL, $this->meta_palabras_clave->ReadOnly);
...
<input type="hidden" data-field="x_id_empresa" name="x_id_empresa" id="x_id_empresa" value="<?php echo ew_HtmlEncode($empresas->id_empresa->CurrentValue) ?>">
