...
$avisos_categorias_delete = NULL; // Initialize page object first

class cavisos_categorias_delete extends cavisos_categorias {
	...
	// Page object name
	var $PageObjName = 'avisos_categorias_delete';
	...
	function Page_Init() {
		...
		if (!$Security->CanDelete()) {
			...
			$this->Page_Terminate(ew_GetUrl("avisos_categorias_list.php"));
		}
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("avisos_categorias_list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in avisos_categorias class, avisos_categoriasinfo.php
...
// No functions
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['id_aviso_categoria'];
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "avisos_categorias_list.php", "", $this->TableVar, TRUE);
		$PageId = "delete";
...
// Create page object
if (!isset($avisos_categorias_delete)) $avisos_categorias_delete = new cavisos_categorias_delete();

// Page init
$avisos_categorias_delete->Page_Init();

// Page main
$avisos_categorias_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$avisos_categorias_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var avisos_categorias_delete = new ew_Page("avisos_categorias_delete");
avisos_categorias_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = avisos_categorias_delete.PageID; // For backward compatibility

// Form object
var favisos_categoriasdelete = new ew_Form("favisos_categoriasdelete");

// Form_CustomValidate event
favisos_categoriasdelete.Form_CustomValidate = 
...
favisos_categoriasdelete.ValidateRequired = true;
<?php } else { ?>
favisos_categoriasdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
favisos_categoriasdelete.Lists["x_id_categoria_nivel1"] = {"LinkField":"x_id_categoria_nivel1","Ajax":null,"AutoFill":false,"DisplayFields":["x_categoria","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
favisos_categoriasdelete.Lists["x_id_categoria_nivel2"] = {"LinkField":"x_id_categoria_nivel2","Ajax":null,"AutoFill":false,"DisplayFields":["x_categoria","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($avisos_categorias_delete->Recordset = $avisos_categorias_delete->LoadRecordset())
	$avisos_categorias_deleteTotalRecs = $avisos_categorias_delete->Recordset->RecordCount(); // Get record count
if ($avisos_categorias_deleteTotalRecs <= 0) { // No record found, exit
	if ($avisos_categorias_delete->Recordset)
		$avisos_categorias_delete->Recordset->Close();
	$avisos_categorias_delete->Page_Terminate("avisos_categorias_list.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $avisos_categorias_delete->ShowPageHeader(); ?>
<?php
$avisos_categorias_delete->ShowMessage();
?>
<form name="favisos_categoriasdelete" id="favisos_categoriasdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($avisos_categorias_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $avisos_categorias_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="avisos_categorias">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($avisos_categorias_delete->RecKeys as $key) { ?>
...
<?php if ($avisos_categorias->id_categoria_nivel1->Visible) { // id_categoria_nivel1 ?>
		<th><span id="elh_avisos_categorias_id_categoria_nivel1" class="avisos_categorias_id_categoria_nivel1"><?php echo $avisos_categorias->id_categoria_nivel1->FldCaption() ?></span></th>
<?php } ?>
<?php if ($avisos_categorias->id_categoria_nivel2->Visible) { // id_categoria_nivel2 ?>
		<th><span id="elh_avisos_categorias_id_categoria_nivel2" class="avisos_categorias_id_categoria_nivel2"><?php echo $avisos_categorias->id_categoria_nivel2->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$avisos_categorias_delete->RecCnt = 0;
$i = 0;
while (!$avisos_categorias_delete->Recordset->EOF) {
	$avisos_categorias_delete->RecCnt++;
	$avisos_categorias_delete->RowCnt++;

	// Set row properties
	$avisos_categorias->ResetAttrs();
	$avisos_categorias->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$avisos_categorias_delete->LoadRowValues($avisos_categorias_delete->Recordset);

	// Render row
	$avisos_categorias_delete->RenderRow();
?>
	<tr<?php echo $avisos_categorias->RowAttributes() ?>>
<?php if ($avisos_categorias->id_categoria_nivel1->Visible) { // id_categoria_nivel1 ?>
		<td<?php echo $avisos_categorias->id_categoria_nivel1->CellAttributes() ?>>
<span id="el<?php echo $avisos_categorias_delete->RowCnt ?>_avisos_categorias_id_categoria_nivel1" class="avisos_categorias_id_categoria_nivel1">
...
<?php } ?>
<?php if ($avisos_categorias->id_categoria_nivel2->Visible) { // id_categoria_nivel2 ?>
		<td<?php echo $avisos_categorias->id_categoria_nivel2->CellAttributes() ?>>
<span id="el<?php echo $avisos_categorias_delete->RowCnt ?>_avisos_categorias_id_categoria_nivel2" class="avisos_categorias_id_categoria_nivel2">
...
<?php
	$avisos_categorias_delete->Recordset->MoveNext();
}
$avisos_categorias_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
favisos_categoriasdelete.Init();
</script>
<?php
$avisos_categorias_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$avisos_categorias_delete->Page_Terminate();
?>
