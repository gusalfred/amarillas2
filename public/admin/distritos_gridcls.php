...
$distritos_grid = NULL; // Initialize page object first

class cdistritos_grid extends cdistritos {
	...
	// Page object name
	var $PageObjName = 'distritos_grid';

	// Grid form hidden field names
	var $FormName = 'fdistritosgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["distritos"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "provincias") {
			global $provincias;
			$rsmaster = $provincias->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("provincias_list.php"); // Return to master page
			} else {
				$provincias->LoadListRowValues($rsmaster);
				$provincias->RowType = EW_ROWTYPE_MASTER; // Master row
				$provincias->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->codigo_distrito->setFormValue($arrKeyFlds[0]);
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->codigo_distrito->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_codigo_distrito") && $objForm->HasValue("o_codigo_distrito") && $this->codigo_distrito->CurrentValue <> $this->codigo_distrito->OldValue)
			return FALSE;
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->codigo_provincia->setSessionValue("");
			}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->codigo_distrito->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->codigo_distrito->setOldValue($objForm->GetValue("o_codigo_distrito"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->codigo_distrito->CurrentValue = strval($arKeys[0]); // codigo_distrito
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->codigo_distrito->EditValue = ew_HtmlEncode($this->codigo_distrito->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->codigo_distrito->FldIsDetailKey && !is_null($this->codigo_distrito->FormValue) && $this->codigo_distrito->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->codigo_distrito->FldCaption(), $this->codigo_distrito->ReqErrMsg));
		}
	...
	function AddRow($rsold = NULL) {
		...
		// codigo_distrito
		$this->codigo_distrito->SetDbValueDef($rsnew, $this->codigo_distrito->CurrentValue, "", FALSE);
		...
		if ($this->codigo_provincia->getSessionValue() <> "") {
			$rsnew['codigo_provincia'] = $this->codigo_provincia->getSessionValue();
		}
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['codigo_distrito']) == "") {
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "provincias") {
			$this->codigo_provincia->Visible = FALSE;
			if ($GLOBALS["provincias"]->EventCancelled) $this->EventCancelled = TRUE;
		}
