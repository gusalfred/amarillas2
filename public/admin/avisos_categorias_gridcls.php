...
$avisos_categorias_grid = NULL; // Initialize page object first

class cavisos_categorias_grid extends cavisos_categorias {
	...
	// Page object name
	var $PageObjName = 'avisos_categorias_grid';

	// Grid form hidden field names
	var $FormName = 'favisos_categoriasgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["avisos_categorias"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_aviso_categoria->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_id_categoria_nivel1") && $objForm->HasValue("o_id_categoria_nivel1") && $this->id_categoria_nivel1->CurrentValue <> $this->id_categoria_nivel1->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_id_categoria_nivel2") && $objForm->HasValue("o_id_categoria_nivel2") && $this->id_categoria_nivel2->CurrentValue <> $this->id_categoria_nivel2->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_aviso_categoria->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('id_aviso_categoria');
	}
	...
	function LoadFormValues() {
		...
		$this->id_categoria_nivel1->setOldValue($objForm->GetValue("o_id_categoria_nivel1"));
		if (!$this->id_categoria_nivel2->FldIsDetailKey) {
			$this->id_categoria_nivel2->setFormValue($objForm->GetValue("x_id_categoria_nivel2"));
		}
		$this->id_categoria_nivel2->setOldValue($objForm->GetValue("o_id_categoria_nivel2"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_aviso_categoria->CurrentValue = strval($arKeys[0]); // id_aviso_categoria
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "avisos") {
			...
			if ($GLOBALS["avisos"]->EventCancelled) $this->EventCancelled = TRUE;
		}
