...
$categorias_nivel2_delete = NULL; // Initialize page object first

class ccategorias_nivel2_delete extends ccategorias_nivel2 {
	...
	// Page object name
	var $PageObjName = 'categorias_nivel2_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("categorias_nivel2_list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in categorias_nivel2 class, categorias_nivel2info.php
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['id_categoria_nivel2'];
...
// Create page object
if (!isset($categorias_nivel2_delete)) $categorias_nivel2_delete = new ccategorias_nivel2_delete();

// Page init
$categorias_nivel2_delete->Page_Init();

// Page main
$categorias_nivel2_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$categorias_nivel2_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var categorias_nivel2_delete = new ew_Page("categorias_nivel2_delete");
categorias_nivel2_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = categorias_nivel2_delete.PageID; // For backward compatibility

// Form object
var fcategorias_nivel2delete = new ew_Form("fcategorias_nivel2delete");

// Form_CustomValidate event
fcategorias_nivel2delete.Form_CustomValidate = 
...
fcategorias_nivel2delete.ValidateRequired = true;
<?php } else { ?>
fcategorias_nivel2delete.ValidateRequired = false; 
<?php } ?>
...
if ($categorias_nivel2_delete->Recordset = $categorias_nivel2_delete->LoadRecordset())
	$categorias_nivel2_deleteTotalRecs = $categorias_nivel2_delete->Recordset->RecordCount(); // Get record count
if ($categorias_nivel2_deleteTotalRecs <= 0) { // No record found, exit
	if ($categorias_nivel2_delete->Recordset)
		$categorias_nivel2_delete->Recordset->Close();
	$categorias_nivel2_delete->Page_Terminate("categorias_nivel2_list.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $categorias_nivel2_delete->ShowPageHeader(); ?>
<?php
$categorias_nivel2_delete->ShowMessage();
?>
<form name="fcategorias_nivel2delete" id="fcategorias_nivel2delete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($categorias_nivel2_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $categorias_nivel2_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="categorias_nivel2">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($categorias_nivel2_delete->RecKeys as $key) { ?>
...
<?php if ($categorias_nivel2->categoria->Visible) { // categoria ?>
		<th><span id="elh_categorias_nivel2_categoria" class="categorias_nivel2_categoria"><?php echo $categorias_nivel2->categoria->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$categorias_nivel2_delete->RecCnt = 0;
$i = 0;
while (!$categorias_nivel2_delete->Recordset->EOF) {
	$categorias_nivel2_delete->RecCnt++;
	$categorias_nivel2_delete->RowCnt++;

	// Set row properties
	$categorias_nivel2->ResetAttrs();
	$categorias_nivel2->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$categorias_nivel2_delete->LoadRowValues($categorias_nivel2_delete->Recordset);

	// Render row
	$categorias_nivel2_delete->RenderRow();
?>
	<tr<?php echo $categorias_nivel2->RowAttributes() ?>>
<?php if ($categorias_nivel2->categoria->Visible) { // categoria ?>
		<td<?php echo $categorias_nivel2->categoria->CellAttributes() ?>>
<span id="el<?php echo $categorias_nivel2_delete->RowCnt ?>_categorias_nivel2_categoria" class="categorias_nivel2_categoria">
...
<?php
	$categorias_nivel2_delete->Recordset->MoveNext();
}
$categorias_nivel2_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fcategorias_nivel2delete.Init();
</script>
<?php
$categorias_nivel2_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$categorias_nivel2_delete->Page_Terminate();
?>
