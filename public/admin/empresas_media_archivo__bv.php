...
$empresas_media_archivo_blobview = NULL; // Initialize page object first

class cempresas_media_archivo_blobview extends cempresas_media {
	...
	// Page object name
	var $PageObjName = 'empresas_media_archivo_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'empresas_media');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('empresas_media', 'x_archivo');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('archivo')));
...
// Create page object
if (!isset($empresas_media_archivo_blobview)) $empresas_media_archivo_blobview = new cempresas_media_archivo_blobview();

// Page init
$empresas_media_archivo_blobview->Page_Init();

// Page main
$empresas_media_archivo_blobview->Page_Main();
?>
<?php
$empresas_media_archivo_blobview->Page_Terminate();
?>
