...
$contratos_add = NULL; // Initialize page object first

class ccontratos_add extends ccontratos {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_contrato"] != "") {
				...
				$this->setKey("id_contrato", $this->id_contrato->CurrentValue); // Set up key
			} else {
				$this->setKey("id_contrato", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "contratos_view.php")
	...
	function LoadDefaultValues() {
		...
		$this->impuesto->CurrentValue = NULL;
		$this->impuesto->OldValue = $this->impuesto->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// fecha_desde
		$this->fecha_desde->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_desde->CurrentValue, 7), NULL, FALSE);

		// fecha_hasta
		$this->fecha_hasta->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_hasta->CurrentValue, 7), NULL, FALSE);
		...
		// impuesto
		$this->impuesto->SetDbValueDef($rsnew, $this->impuesto->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->id_contrato->setDbValue($conn->Insert_ID());
			$rsnew['id_contrato'] = $this->id_contrato->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($contratos_add)) $contratos_add = new ccontratos_add();

// Page init
$contratos_add->Page_Init();

// Page main
$contratos_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$contratos_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var contratos_add = new ew_Page("contratos_add");
contratos_add.PageID = "add"; // Page ID
var EW_PAGE_ID = contratos_add.PageID; // For backward compatibility

// Form object
var fcontratosadd = new ew_Form("fcontratosadd");

// Validate form
fcontratosadd.Validate = function() {
...
fcontratosadd.Form_CustomValidate = 
...
fcontratosadd.ValidateRequired = true;
<?php } else { ?>
fcontratosadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcontratosadd.Lists["x_id_empresa"] = {"LinkField":"x_id_empresa","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcontratosadd.Lists["x_id_plan"] = {"LinkField":"x_id_plan","Ajax":null,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcontratosadd.Lists["x_id_empleado"] = {"LinkField":"x_id_empleado","Ajax":null,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};

// Form object for search
</script>
<script type="text/javascript">
$(document).ready(function() {
$('#r_monto').before('<div id="vue"></div>');
$('#vue').prepend( $('#r_monto, #r_descuento, #r_sub_total, #r_impuesto, #r_monto_total') );
var vm = new Vue({
  el: '#vue',
  data: {
	monto: '0',
...
$('#x_id_plan').on('change', function() {
	$.ajax("plan.php?id="+this.value)
	.done(function(valor){
	vm.monto = valor;
	});  
})
...
<?php $contratos_add->ShowPageHeader(); ?>
<?php
$contratos_add->ShowMessage();
?>
<form name="fcontratosadd" id="fcontratosadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($contratos_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $contratos_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fcontratosadd.CreateAutoSuggest("x_id_empresa", false);
</script>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fcontratosadd", "x_fecha", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
fcontratosadd.Lists["x_id_plan"].Options = <?php echo (is_array($contratos->id_plan->EditValue)) ? ew_ArrayToJson($contratos->id_plan->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fcontratosadd", "x_fecha_desde", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fcontratosadd", "x_fecha_hasta", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
fcontratosadd.Lists["x_id_empleado"].Options = <?php echo (is_array($contratos->id_empleado->EditValue)) ? ew_ArrayToJson($contratos->id_empleado->EditValue, 1) : "[]" ?>;
</script>
</span>
