...
$avisos_url_aviso_blobview = NULL; // Initialize page object first

class cavisos_url_aviso_blobview extends cavisos {
	...
	// Page object name
	var $PageObjName = 'avisos_url_aviso_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'avisos');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('avisos', 'x_url_aviso');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('url_aviso');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('url_aviso')));
				if (trim(strval($this->Recordset->fields('url_aviso'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('url_aviso') . "\"");
				}
...
// Create page object
if (!isset($avisos_url_aviso_blobview)) $avisos_url_aviso_blobview = new cavisos_url_aviso_blobview();

// Page init
$avisos_url_aviso_blobview->Page_Init();

// Page main
$avisos_url_aviso_blobview->Page_Main();
?>
<?php
$avisos_url_aviso_blobview->Page_Terminate();
?>
