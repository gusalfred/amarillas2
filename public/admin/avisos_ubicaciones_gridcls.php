...
$avisos_ubicaciones_grid = NULL; // Initialize page object first

class cavisos_ubicaciones_grid extends cavisos_ubicaciones {
	...
	// Page object name
	var $PageObjName = 'avisos_ubicaciones_grid';

	// Grid form hidden field names
	var $FormName = 'favisos_ubicacionesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["avisos_ubicaciones"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_aviso_ubicacion->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_aviso_ubicacion->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('id_aviso_ubicacion');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_aviso_ubicacion->CurrentValue = strval($arKeys[0]); // id_aviso_ubicacion
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
