...
$provincias_grid = NULL; // Initialize page object first

class cprovincias_grid extends cprovincias {
	...
	// Page object name
	var $PageObjName = 'provincias_grid';

	// Grid form hidden field names
	var $FormName = 'fprovinciasgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["provincias"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->codigo_provincia->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_codigo_provincia") && $objForm->HasValue("o_codigo_provincia") && $this->codigo_provincia->CurrentValue <> $this->codigo_provincia->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->codigo_provincia->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('codigo_provincia');
	}
	...
	function LoadFormValues() {
		...
		$this->codigo_provincia->setOldValue($objForm->GetValue("o_codigo_provincia"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->codigo_provincia->CurrentValue = strval($arKeys[0]); // codigo_provincia
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "departamentos") {
			...
			if ($GLOBALS["departamentos"]->EventCancelled) $this->EventCancelled = TRUE;
		}
