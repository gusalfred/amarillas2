...
$categorias_nivel2_grid = NULL; // Initialize page object first

class ccategorias_nivel2_grid extends ccategorias_nivel2 {
	...
	// Page object name
	var $PageObjName = 'categorias_nivel2_grid';

	// Grid form hidden field names
	var $FormName = 'fcategorias_nivel2grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["categorias_nivel2"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_categoria_nivel2->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_categoria_nivel2->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('id_categoria_nivel2');
	}
	...
	function LoadFormValues() {
		...
		if (!$this->id_categoria_nivel2->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_categoria_nivel2->CurrentValue = strval($arKeys[0]); // id_categoria_nivel2
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "categorias_nivel1") {
			$this->id_categoria_nivel1->Visible = FALSE;
			if ($GLOBALS["categorias_nivel1"]->EventCancelled) $this->EventCancelled = TRUE;
		}
