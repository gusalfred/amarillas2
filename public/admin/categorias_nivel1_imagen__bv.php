...
$categorias_nivel1_imagen_blobview = NULL; // Initialize page object first

class ccategorias_nivel1_imagen_blobview extends ccategorias_nivel1 {
	...
	// Page object name
	var $PageObjName = 'categorias_nivel1_imagen_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'categorias_nivel1');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('categorias_nivel1', 'x_imagen');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('imagen')));
				if (trim(strval($this->Recordset->fields('imagen'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('imagen') . "\"");
				}
...
// Create page object
if (!isset($categorias_nivel1_imagen_blobview)) $categorias_nivel1_imagen_blobview = new ccategorias_nivel1_imagen_blobview();

// Page init
$categorias_nivel1_imagen_blobview->Page_Init();

// Page main
$categorias_nivel1_imagen_blobview->Page_Main();
?>
<?php
$categorias_nivel1_imagen_blobview->Page_Terminate();
?>
