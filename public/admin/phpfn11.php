...
function ew_AutoLoad($class) {
	...
	if (substr($class, 0, 1) == "c") {
		if ($class == "camarillas365_db") {
			$file = "amarillas365db.php";
		} else {
		$fn = "%cls%_info.php";
...
function CurrentProjectID() {
	...
	return "{D02329FA-C783-46FA-A3B4-996FD9449799}";
}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('clave'), $pwd);
					...
					if ($ValidateUser) {
						...
						if (is_null($rs->fields('id_empleado_nivel'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('id_empleado_nivel'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function SetUpUserLevelEx() {
		...
		if (count($this->UserLevelPriv) == 0 && $this->IsAdmin() && $Page != NULL && @$_SESSION[EW_SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->Page_Terminate("empleados_niveles_list.php");
		}
		return TRUE;
	}
...
class cUserProfile {
	...
	function LoadProfileFromDatabase($usr) {
		...
		if ($usr == "" || IsSysAdmin()) // Ignore hard code admin
			return FALSE;
		$sFilter = str_replace("%u", ew_AdjustSql($usr), EW_USER_NAME_FILTER);
