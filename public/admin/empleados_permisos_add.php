...
$empleados_permisos_add = NULL; // Initialize page object first

class cempleados_permisos_add extends cempleados_permisos {
	...
	// Page object name
	var $PageObjName = 'empleados_permisos_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_usuario_nivel"] != "") {
				...
				$this->setKey("id_usuario_nivel", $this->id_usuario_nivel->CurrentValue); // Set up key
			} else {
				$this->setKey("id_usuario_nivel", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "empleados_permisos_view.php")
	...
	function LoadDefaultValues() {
		$this->id_usuario_nivel->CurrentValue = NULL;
		$this->id_usuario_nivel->OldValue = $this->id_usuario_nivel->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->id_usuario_nivel->EditValue = ew_HtmlEncode($this->id_usuario_nivel->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		// id_usuario_nivel
		$this->id_usuario_nivel->SetDbValueDef($rsnew, $this->id_usuario_nivel->CurrentValue, 0, FALSE);
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['id_usuario_nivel']) == "") {
...
// Create page object
if (!isset($empleados_permisos_add)) $empleados_permisos_add = new cempleados_permisos_add();

// Page init
$empleados_permisos_add->Page_Init();

// Page main
$empleados_permisos_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$empleados_permisos_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var empleados_permisos_add = new ew_Page("empleados_permisos_add");
empleados_permisos_add.PageID = "add"; // Page ID
var EW_PAGE_ID = empleados_permisos_add.PageID; // For backward compatibility

// Form object
var fempleados_permisosadd = new ew_Form("fempleados_permisosadd");

// Validate form
fempleados_permisosadd.Validate = function() {
...
fempleados_permisosadd.Form_CustomValidate = 
...
fempleados_permisosadd.ValidateRequired = true;
<?php } else { ?>
fempleados_permisosadd.ValidateRequired = false; 
<?php } ?>
...
<?php $empleados_permisos_add->ShowPageHeader(); ?>
<?php
$empleados_permisos_add->ShowMessage();
?>
<form name="fempleados_permisosadd" id="fempleados_permisosadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($empleados_permisos_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $empleados_permisos_add->Token ?>">
<?php } ?>
...
<input type="text" data-field="x_id_usuario_nivel" name="x_id_usuario_nivel" id="x_id_usuario_nivel" size="30" value="<?php echo $empleados_permisos->id_usuario_nivel->EditValue ?>"<?php echo $empleados_permisos->id_usuario_nivel->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_tabla" name="x_tabla" id="x_tabla" size="30" maxlength="255" value="<?php echo $empleados_permisos->tabla->EditValue ?>"<?php echo $empleados_permisos->tabla->EditAttributes() ?>>
</span>
...
<script type="text/javascript">
fempleados_permisosadd.Init();
</script>
<?php
$empleados_permisos_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$empleados_permisos_add->Page_Terminate();
?>
