...
class cavisos extends cTable {
	...
	function __construct() {
		...
		// nombre_aviso
		$this->nombre_aviso = new cField('avisos', 'avisos', 'x_nombre_aviso', 'nombre_aviso', '`nombre_aviso`', '`nombre_aviso`', 200, -1, FALSE, '`nombre_aviso`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nombre_aviso'] = &$this->nombre_aviso;

		// archivo
		$this->archivo = new cField('avisos', 'avisos', 'x_archivo', 'archivo', '`archivo`', '`archivo`', 200, -1, TRUE, '`archivo`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['archivo'] = &$this->archivo;

		// url
		$this->url = new cField('avisos', 'avisos', 'x_url', 'url', '`url`', '`url`', 200, -1, FALSE, '`url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "avisos_categorias") {
			$sDetailUrl = $GLOBALS["avisos_categorias"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_id_aviso=" . urlencode($this->id_aviso->CurrentValue);
		}
		if ($sDetailUrl == "") {
	...
	function Update(&$rs, $where = "", $rsold = NULL) {
		...
		// Cascade Update detail table 'avisos_ubicaciones'
		$bCascadeUpdate = FALSE;
		$rscascade = array();
		if (!is_null($rsold) && (isset($rs['id_aviso']) && $rsold['id_aviso'] <> $rs['id_aviso'])) { // Update detail field 'id_aviso'
			$bCascadeUpdate = TRUE;
			$rscascade['id_aviso'] = $rs['id_aviso']; 
		}
		if ($bCascadeUpdate) {
			if (!isset($GLOBALS["avisos_ubicaciones"])) $GLOBALS["avisos_ubicaciones"] = new cavisos_ubicaciones();
			$GLOBALS["avisos_ubicaciones"]->Update($rscascade, "`id_aviso` = " . ew_QuotedValue($rsold['id_aviso'], EW_DATATYPE_NUMBER));
		}
	...
	function Delete(&$rs, $where = "") {
		...
		// Cascade delete detail table 'avisos_ubicaciones'
		if (!isset($GLOBALS["avisos_ubicaciones"])) $GLOBALS["avisos_ubicaciones"] = new cavisos_ubicaciones();
		$rscascade = array();
		$GLOBALS["avisos_ubicaciones"]->Delete($rscascade, "`id_aviso` = " . ew_QuotedValue($rs['id_aviso'], EW_DATATYPE_NUMBER));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nombre_aviso->Exportable) $Doc->ExportCaption($this->nombre_aviso);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->nombre_aviso->Exportable) $Doc->ExportField($this->nombre_aviso);
