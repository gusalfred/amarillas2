...
$categorias_nivel1 = NULL;

//
// Table class for categorias_nivel1
//
class ccategorias_nivel1 extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'categorias_nivel1';
		$this->TableName = 'categorias_nivel1';
		...
		// id_categoria_nivel1
		$this->id_categoria_nivel1 = new cField('categorias_nivel1', 'categorias_nivel1', 'x_id_categoria_nivel1', 'id_categoria_nivel1', '`id_categoria_nivel1`', '`id_categoria_nivel1`', 19, -1, FALSE, '`id_categoria_nivel1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// categoria
		$this->categoria = new cField('categorias_nivel1', 'categorias_nivel1', 'x_categoria', 'categoria', '`categoria`', '`categoria`', 200, -1, FALSE, '`categoria`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['categoria'] = &$this->categoria;

		// descripcion
		$this->descripcion = new cField('categorias_nivel1', 'categorias_nivel1', 'x_descripcion', 'descripcion', '`descripcion`', '`descripcion`', 200, -1, FALSE, '`descripcion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['descripcion'] = &$this->descripcion;

		// slug
		$this->slug = new cField('categorias_nivel1', 'categorias_nivel1', 'x_slug', 'slug', '`slug`', '`slug`', 200, -1, FALSE, '`slug`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['slug'] = &$this->slug;

		// imagen
		$this->imagen = new cField('categorias_nivel1', 'categorias_nivel1', 'x_imagen', 'imagen', '`imagen`', '`imagen`', 200, -1, TRUE, '`imagen`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['imagen'] = &$this->imagen;

		// orden
		$this->orden = new cField('categorias_nivel1', 'categorias_nivel1', 'x_orden', 'orden', '`orden`', '`orden`', 19, -1, FALSE, '`orden`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "categorias_nivel2") {
			$sDetailUrl = $GLOBALS["categorias_nivel2"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_id_categoria_nivel1=" . urlencode($this->id_categoria_nivel1->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "categorias_nivel1_list.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`categorias_nivel1`";
	}
	...
	// Update Table
	var $UpdateTable = "`categorias_nivel1`";
	...
	function Update(&$rs, $where = "", $rsold = NULL) {
		...
		// Cascade Update detail table 'categorias_nivel2'
		$bCascadeUpdate = FALSE;
		$rscascade = array();
		if (!is_null($rsold) && (isset($rs['id_categoria_nivel1']) && $rsold['id_categoria_nivel1'] <> $rs['id_categoria_nivel1'])) { // Update detail field 'id_categoria_nivel1'
			$bCascadeUpdate = TRUE;
			$rscascade['id_categoria_nivel1'] = $rs['id_categoria_nivel1']; 
		}
		if ($bCascadeUpdate) {
			if (!isset($GLOBALS["categorias_nivel2"])) $GLOBALS["categorias_nivel2"] = new ccategorias_nivel2();
			$GLOBALS["categorias_nivel2"]->Update($rscascade, "`id_categoria_nivel1` = " . ew_QuotedValue($rsold['id_categoria_nivel1'], EW_DATATYPE_NUMBER));
		}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('id_categoria_nivel1', $rs))
				ew_AddFilter($where, ew_QuotedName('id_categoria_nivel1') . '=' . ew_QuotedValue($rs['id_categoria_nivel1'], $this->id_categoria_nivel1->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function Delete(&$rs, $where = "") {
		...
		// Cascade delete detail table 'categorias_nivel2'
		if (!isset($GLOBALS["categorias_nivel2"])) $GLOBALS["categorias_nivel2"] = new ccategorias_nivel2();
		$rscascade = array();
		$GLOBALS["categorias_nivel2"]->Delete($rscascade, "`id_categoria_nivel1` = " . ew_QuotedValue($rs['id_categoria_nivel1'], EW_DATATYPE_NUMBER));
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_categoria_nivel1->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_categoria_nivel1@", ew_AdjustSql($this->id_categoria_nivel1->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "categorias_nivel1_list.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("categorias_nivel1_view.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("categorias_nivel1_view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "categorias_nivel1_add.php?" . $this->UrlParm($parm);
		else
			return "categorias_nivel1_add.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("categorias_nivel1_edit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("categorias_nivel1_edit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("categorias_nivel1_add.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("categorias_nivel1_add.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("categorias_nivel1_delete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_categoria_nivel1->CurrentValue)) {
			$sUrl .= "id_categoria_nivel1=" . urlencode($this->id_categoria_nivel1->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_categoria_nivel1"]; // id_categoria_nivel1
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_categoria_nivel1->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->id_categoria_nivel1->EditValue = $this->id_categoria_nivel1->CurrentValue;
		$this->id_categoria_nivel1->ViewCustomAttributes = "";
