...
class cempleados_view extends cempleados {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["id_empleado"] <> "") {
				$this->id_empleado->setFormValue($_POST["id_empleado"]);
				$this->RecKey["id_empleado"] = $this->id_empleado->FormValue;
			} else {
				$sReturnUrl = "empleados_list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "empleados_list.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "empleados_list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->id_usuario_nivel->CurrentValue) <> "") {
				$sFilterWrk = "`id_usuario_nivel`" . ew_SearchString("=", $this->id_usuario_nivel->CurrentValue, EW_DATATYPE_NUMBER);
			$sSqlWrk = "SELECT `id_usuario_nivel`, `nivel` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `empleados_niveles`";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
			...
			$this->Lookup_Selecting($this->id_usuario_nivel, $sWhereWrk);
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->id_usuario_nivel->ViewValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->id_usuario_nivel->ViewValue = $this->id_usuario_nivel->CurrentValue;
				}
			} else {
				$this->id_usuario_nivel->ViewValue = NULL;
			}
			} else {
				$this->id_usuario_nivel->ViewValue = "********";
			}
...
fempleadosview.Lists["x_id_usuario_nivel"] = {"LinkField":"x_id_usuario_nivel","Ajax":null,"AutoFill":false,"DisplayFields":["x_nivel","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fempleadosview" id="fempleadosview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($empleados_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $empleados_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="empleados">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($empleados->id_usuario_nivel->Visible) { // id_usuario_nivel ?>
	<tr id="r_id_usuario_nivel">
		<td><span id="elh_empleados_id_usuario_nivel"><?php echo $empleados->id_usuario_nivel->FldCaption() ?></span></td>
		<td<?php echo $empleados->id_usuario_nivel->CellAttributes() ?>>
<span id="el_empleados_id_usuario_nivel">
<span<?php echo $empleados->id_usuario_nivel->ViewAttributes() ?>>
<?php echo $empleados->id_usuario_nivel->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empleados->nombre->Visible) { // nombre ?>
	<tr id="r_nombre">
		<td><span id="elh_empleados_nombre"><?php echo $empleados->nombre->FldCaption() ?></span></td>
		<td<?php echo $empleados->nombre->CellAttributes() ?>>
<span id="el_empleados_nombre">
<span<?php echo $empleados->nombre->ViewAttributes() ?>>
<?php echo $empleados->nombre->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empleados->usuario->Visible) { // usuario ?>
	<tr id="r_usuario">
		<td><span id="elh_empleados_usuario"><?php echo $empleados->usuario->FldCaption() ?></span></td>
		<td<?php echo $empleados->usuario->CellAttributes() ?>>
<span id="el_empleados_usuario">
<span<?php echo $empleados->usuario->ViewAttributes() ?>>
<?php echo $empleados->usuario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empleados->clave->Visible) { // clave ?>
	<tr id="r_clave">
		<td><span id="elh_empleados_clave"><?php echo $empleados->clave->FldCaption() ?></span></td>
		<td<?php echo $empleados->clave->CellAttributes() ?>>
<span id="el_empleados_clave">
<span<?php echo $empleados->clave->ViewAttributes() ?>>
<?php echo $empleados->clave->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
